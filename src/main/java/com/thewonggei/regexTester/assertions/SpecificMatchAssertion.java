 ...
 * @since 0.2
 */
public class SpecificMatchAssertion extends AbstractRegexAssertion {
	private final int matchNumber;
	private final String testString;
	private final String inputString;
	private final boolean shouldItMatch;
	
	public SpecificMatchAssertion(final int matchNumber, final String testString, final String inputString, final boolean shouldItMatch) {
	...
	public int compareTo(RegexAssertion compareThis) {
		if( !(compareThis instanceof SpecificMatchAssertion) &&
		...
		else if( !(compareThis instanceof SpecificMatchAssertion) ) {
			throw new IllegalArgumentException("Can only compare against type SpecificMatchAssertion.");
		}

		SpecificMatchAssertion compareMA = (SpecificMatchAssertion)compareThis;
		int compareResult = 0;
